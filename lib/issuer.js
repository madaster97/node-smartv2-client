...
const {
  ...
  FHIR_DISCOVERY, OIDC_CAPABILITY,
...
class Issuer {
   ...
   * @api public
   */
  static async fhirDiscover(uri) {
    const parsed = url.parse(uri);

    if (!parsed.pathname.includes('/.well-known/')) {
      parsed.pathname = (parsed.pathname.endsWith('/'))
        ? `${parsed.pathname}${FHIR_DISCOVERY.substring(1)}` : `${parsed.pathname}${FHIR_DISCOVERY}`;
    }
    ...
    const capable = Array.isArray(body.capabilities) && body.capabilities.includes(OIDC_CAPABILITY);

    if (!capable) {
      throw new RPError({
        message: 'Fhir server doesn\'t claim to be sso-openid-connect capable',
        body,
      });
    }
    ...
    if (!body.issuer) {
      throw new RPError({
        message: 'Fhir server did not present an issuer url',
        body,
      });
    }
    ...
    if (typeof body.issuer !== 'string' || !body.issuer.startsWith('https://')) {
      throw new RPError({
        printf: ['invalid issuer location %s', body.issuer],
        body,
      });
    }

    return new Promise((res, rej) => {
      this.discover(body.issuer).then((issuer) => {
        res(new Issuer({ ...issuer.metadata, fhirCapabilities: body.capabilities, iss: uri }));
      }).catch(rej);
    });
  }
